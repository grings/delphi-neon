      ...
      begin
        if AType.Handle = TypeInfo(Single) then
          ...
          if AType.Handle = TypeInfo(TDateTime) then
            ...
            if AType.Handle = TypeInfo(TTime) then
              Result := WriteDateTime(AType, ANeonObject)
            else
    ...
    tkClass:
      begin
        if AType.IsInstance and
          ...
          if AType.IsInstance and
             AType.AsInstance.MetaclassType.InheritsFrom(TStream) then
            ...
            if IsEnumerableMap(AType, LNeonMap) then
              ...
              if IsEnumerable(AType, LNeonList) then
                Result := WriteEnumerable(AType, ANeonObject, LNeonList)
              else
                if IsStreamable(AType, LNeonStream) then
  ...
  begin
    ...
    begin
      try
        ...
        LogError(Format('Error converting property [%s] of object [%s]', [LNeonMember.Name, AType.Name]));
      end;
    end;
  end;
end;
